/**
 * Pinellas County Property Appraiser Scraper
 * URL Pattern: https://www.pcpao.gov/property-details?s={parcelId}
 * Note: Owner names and address are in specific ID'd elements
 */
...
export default class PinellasScraper extends BaseScraper {
  /**
   * Pinellas uses direct URL navigation with s parameter
   */
  protected async navigateToSearch(page: Page, request?: ScrapeRequest): Promise<void> {
    if (!request) {
      throw new ScraperError(
        ErrorCode.VALIDATION_ERROR,
        'Request is required for Pinellas County navigation',
        undefined,
        this.config.id
      )
    }
  ...
  async scrape(request: ScrapeRequest): Promise<any> {
    ...
    try {
      ...
      await page.waitForSelector('#first_second_owner', { timeout: this.config.timeout || 10000 })
  ...
  /**
   * Pinellas doesn't need a search step - we navigate directly to the property page
   */
  ...
  /**
   * Extract property data from Pinellas property details page
   * Owner names are in span#first_second_owner
   * Mailing address is in div#mailling_add
   */
  protected async extractPropertyData(
    ...
    try {
      ...
      const data = await page.evaluate(() => {
        ...
        // Find the owner names span
        const ownerSpan = document.querySelector('#first_second_owner')
        if (ownerSpan) {
          ownerName = htmlToText(ownerSpan.innerHTML)
        }

        // Find the mailing address div
        const addressDiv = document.querySelector('#mailling_add')
        if (addressDiv) {
          mailingAddress = htmlToText(addressDiv.innerHTML)
        }
